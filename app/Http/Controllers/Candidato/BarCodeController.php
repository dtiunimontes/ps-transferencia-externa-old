...
class BarCodeController extends Controller{
	...
	function __construct($value, $into = 1, $filename = 'barcode.gif', $width_bar = 300, $height_bar = 65, $show_codebar = false, $nome){
        ...
        if ($show_codebar) {
            imagestring($img, 1, 0, $height_bar+2, " ".$value_imp." ".utf8_encode($nome), imagecolorallocate($img, 0, 0, 0));
        }
        $this->put_img($img, $value_imp);
    }
    ...
    function put_img($image, $value, $file='test.gif'){
        if($this->into){
            imagegif($image,$this->file);
        } else {
            header("Content-type: image/png");
            $img = imagepng($image, "etiquetas/".$value.".png");
        }
        imagedestroy($image);
    }
}
